...
ifneq ($(IN_NESTED_DOCKER),yes)
	...
	docker run --rm -ti -e IN_NESTED_DOCKER=yes $(DOCKER_OPTS) -v "$(PWD)":/$(PROJECT_DIR) --workdir /$(PROJECT_DIR) $(_DOCKER_IMAGE_) make IN_NESTED_DOCKER=yes -f $(firstword $(MAKEFILE_LIST)) $(MAKECMDGOALS);
	#
